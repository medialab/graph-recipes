...
var json_graph_api = {
	parseGEXF: function(gexf){
		...
		var viz_1_2="http://www.gexf.net/1.2draft/viz";	// Vis namespace
		var viz="http://www.gexf.net/1.3/viz";
		...
		for(i=0; i<edgesNodes.length; i++){
			...
			for(ii=0; ii<edgeNodes.length; ii++){
				...
				var edge = {id:ii, sourceID:source, targetID:target, attributes:[], weight:+weight || 1};
	...
	buildGEXF: function(graph){
		...
		graph.edges.forEach(function(edge){
			...
			var weight = json_graph_api.xmlEntities(edge.weight || 1);
			
			content.push("\n" +  '<edge source="'+sourceId+'" target="'+targetId+'" weight="'+weight+'" >');
